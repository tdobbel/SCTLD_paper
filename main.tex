...
\end{equation}
These equations give a reasonable approximation of the evolution of the disease on sub-reefs for which $I_j > I_0$, as intra-reef infection, when activated, is the dominant infection mechanism. Using this approximation, the ratio $\beta_0'/\sigma$ is imposed by matching the modeled proportion of susceptible corals remaining after the disease has vanished ($S_\infty$) with observations. A standard property of a SIR model solution is that
\begin{equation}
...
\begin{itemize}
    \item Disease transmitted in about 6.45 days while infectious corals are removed in 6.99 days
    \item Mean barotropic currents are the most likey driver of the causative agent of the disease
    \item Propagation extremely sensitive to the value of the infection threshold $I_0$ with realistic spread of the disease occurring for threshold values between 0.05 and 0.1 \%    
\end{itemize}

%% --- DISCUSSION OF TAKE HOME MESSAGES/MAIN RESULTS --- %%

% 1. MEAN BAROTROPIC CURRENTS => NEUTRALLY BUOYANT VECTOR 
...
% 2. IMPACT OF I0/APPEARANCE OF PLATEAU
...
% subdivision into highly/intermediately/weakly susceptible species
